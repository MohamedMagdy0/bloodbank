...
@section('content')
                   ...
                   @foreach ($categories as $category)
                       ...
                       <tr>
                          <td>{{ $categories->FirstItem()+$loop->index}}</td>
                           <td>{{ $category->name }}</td>
                           <td><a href="{{ route('categories.edit', ['category'=>$category->id]) }}" class="btn btn-sm btn-success">Edit</a></td>
   ...
   <div class="modal fade" id="modal-default{{ $category->id }}">
    <!-- start form -->
    <form action="{{ route('categories.softDelete', ['id'=>$category->id]) }}" method="post">
        @csrf
        @method('DELETE')
              ...
              <button type="submit" class="btn btn-default">Delete</button>
                ...
                @else
                   <div class="alert alert-dark text-center ">
                        <h1 class="text-capitalize">Sorry No Categories</h1>
                        <a class="linked text-primary" href="{{ route('categories.create') }}">If You Want To Create Please Click Here</a>
                   </div><!-- alert elseif -->
                @endif
