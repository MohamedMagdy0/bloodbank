...
@section('content')
    <section class="container">
        <div class="card">
            <div class="card-header text-center">
                <h3>All BloodTypes : {{ count($bloodTypes) }}</h3>
            </div><!-- card-header -->
                   ...
                   @foreach ($bloodTypes as $bloodType)
                       ...
                       <tr>
                          <td>{{ $bloodTypes->FirstItem()+$loop->index}}</td>
                           ...
                           <td><a href="{{ route('blood-types.edit', ['blood_type'=>$bloodType->id]) }}" class="btn btn-sm btn-success">Edit</a></td>
   ...
   <div class="modal fade" id="modal-default{{ $bloodType->id }}">
    <!-- start form -->
    <form action="{{ route('blood-types.softDelete', ['id'=>$bloodType->id]) }}" method="post">
        @csrf
        @method('DELETE')
              ...
              <button type="submit" class="btn btn-default">Delete</button>
                ...
                @else
                   <div class="alert alert-dark text-center ">
                        <h1 class="text-capitalize">Sorry No BloodTypes</h1>
                        <a class="linked text-primary" href="{{ route('blood-types.create') }}">If You Want To Create Please Click Here</a>
                   </div><!-- alert elseif -->
                @endif
